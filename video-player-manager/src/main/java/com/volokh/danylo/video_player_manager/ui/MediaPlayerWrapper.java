...
public abstract class MediaPlayerWrapper
    ...
    private String TAG = "MediaPlayerWrapper";
...
//        Logger.e("mediaPlayerWrapper -- onPrepared ");
        if (mListener != null) {
...
//                    throw new IllegalStateException("prepare, called from illegal state " + mState);
                    ...
                    onPrepareError(new IllegalStateException());
            }
        }
        if (SHOW_LOGS) Logger.v(TAG, "<< prepare, mState " + mState);
    }
     ...
     * @param ex
     */
    private void onPrepareError(Exception ex) {
    ...
    private void printInfo(int what) {
        switch (what) {
            ...
            case MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START:
                //缓存完成，开始播放
                if (mListener != null) {
                    mListener.onBufferingUpdateMainThread(-100);
                }else{
                    Logger.e("--- mListener = null");
                }
                if (SHOW_LOGS) Logger.inf(TAG, "onInfo, MEDIA_INFO_VIDEO_RENDERING_START");
                break;
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                if (mListener != null) {
                    mListener.onBufferingUpdateMainThread(-200);
                }else{
                    Logger.e("--- mListener = null");
                }
...
//                    throw new IllegalStateException("start, called from illegal state " + mState);
                    Logger.e("start, called from illegal state " +mState);
                    break;
                case STOPPED:
                case PLAYBACK_COMPLETED:
                case PREPARED:
                ...
                case INITIALIZED:
                    ...
                    mMediaPlayer.start();
//                    Logger.e("mediaPlayerWrapper -- start ");
...
//                        public void run() {
//                            if(mListener != null){
//                                mListener.onVideoPlayTimeChanged(0);
//                            }
//                        }
//                    },500);
                    break;
...
//                    Logger.e("停止播放！");
     ...
     * @param i
     */
    public void seekToShoudong(long i) {
        seekToPercent(i);
    }
...
//                    notifyPositionUpdated();
                    Logger.e("更新进度--"+positionMillis/1000);
                    break;
            }
        }
    }
...
//        if (SHOW_LOGS)
//            Logger.e( "stopPositionUpdateNotifier, mPositionUpdateNotifier " + mPositionUpdateNotifier);
...
//            if (SHOW_LOGS) Logger.v(TAG, "notifyPositionUpdated, mVideoStateListener " + mVideoStateListener);
            if(mState.get() == State.STARTED ){
                mMainThreadHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        try{
                            if(mListener != null){
                                mListener.onVideoPlayTimeChanged(mMediaPlayer.getCurrentPosition());
                            }
                        }catch (IllegalStateException e){
                            Logger.e("进度更新状态异常  --"+ mState.get());
                        }
                    }
                });
            }
    ...
    public interface MainThreadMediaPlayerListener {
        ...
        void onVideoPlayTimeChanged(int currentPosition);
    }
