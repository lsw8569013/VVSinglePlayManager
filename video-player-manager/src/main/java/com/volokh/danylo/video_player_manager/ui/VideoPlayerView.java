    ...
    ,CacheListener
{
    ...
    private MessagesHandlerThread playHandler;
    private SingleVideoPlayerManager playManager;
    ...
    public void seekTo(long progress) {
        mMediaPlayer.seekToPercent(progress);
    }

    public void startSeekTo(long time ){
        try {
            if(mMediaPlayer != null && time>=0){
                mMediaPlayer.startSeekTo(time);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void setPlayHandler(MessagesHandlerThread playHandler) {
        this.playHandler = playHandler;
    }

    public void setPlayManager(SingleVideoPlayerManager playManager) {
...
//            timerStart();
    ...
    public void start(float volumeNum ){
        if (SHOW_LOGS) Logger.e( "videoPlayerView --->> start volumeNum");
        synchronized (mReadyForPlaybackIndicator){
            if(mReadyForPlaybackIndicator.isReadyForPlayback() && VideoPlayerView.this.getVisibility() == View.VISIBLE){
                ...
                if(!mReadyForPlaybackIndicator.isFailedToPrepareUiForPlayback()){
                    ...
                    if(mReadyForPlaybackIndicator.isReadyForPlayback()){
                        ...
                        if (SHOW_LOGS) Logger.e( "start, movie is not ready, Player become STARTED state, but it will actually don't play");
                    }
                } else {
                    if (SHOW_LOGS) Logger.e( "start, movie is not ready. Video size will not become available");
                }
            }
        }
...
//            if (SHOW_LOGS) Logger.err(TAG, "setDataSource, path " + path + ", this " + this);

            try {
                mMediaPlayer.setDataSource(path);

            } catch (IOException e) {
                Logger.e("setDataSource error --"+ e.getMessage());
//                throw new RuntimeException(e);
            }
            mPath = path;
        }
...
//                throw new RuntimeException(e);
                mMediaPlayer.onError(mMediaPlayer.getMediaPlayer(),-1,-1);
            }
    ...
    public void onVideoPreparedMainThread() {
        ...
        //播放
        playManager.addStartMessage(this);
        if (mMediaPlayerListenerBackgroundThread != null) {
...
//        if (SHOW_LOGS) Logger.e("onErrorMainThread, this " + VideoPlayerView.this);
        switch (what) {
...
//        if(percent != 100 ){
//            notifyOnVideoBufferUpdate(percent);
//        }
...
//        Logger.e("playview cache -- " + percentsAvailable);
        notifyOnVideoBufferUpdate(percentsAvailable);
    }

    private void notifyOnVideoBufferUpdate(int percent) {
        List<MediaPlayerWrapper.MainThreadMediaPlayerListener> listCopy;
        synchronized (mMediaPlayerMainThreadListeners){
            listCopy = new ArrayList<>(mMediaPlayerMainThreadListeners);
        }
...
//                if (SHOW_LOGS) Logger.e( "error extra MEDIA_ERROR_IO");
                break;
            case MediaPlayer.MEDIA_ERROR_MALFORMED:
//                if (SHOW_LOGS) Logger.e( "error extra MEDIA_ERROR_MALFORMED");
                break;
            case MediaPlayer.MEDIA_ERROR_UNSUPPORTED:
                if (SHOW_LOGS) Logger.e("error extra MEDIA_ERROR_UNSUPPORTED");
                break;
            case MediaPlayer.MEDIA_ERROR_TIMED_OUT:
                if (SHOW_LOGS) Logger.e("error extra MEDIA_ERROR_TIMED_OUT");
                break;
        }
    }
...
//        if (SHOW_LOGS) Logger.v(TAG, "onSurfaceTextureUpdated, mIsVideoStartedCalled " + mIsVideoStartedCalled.get() + ", mMediaPlayer.getState() " + mMediaPlayer.getState());
    ...
    public void onVideoPlayTimeChanged(int positionInMilliseconds) {
        ...
        for (MediaPlayerWrapper.MainThreadMediaPlayerListener listener : listCopy){
            listener.onVideoPlayTimeChanged(positionInMilliseconds);
        }
    }
